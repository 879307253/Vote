package com.example.a87939.vote;
...
//存储处理结果
    ...
    private String doVote(String voteStr){ String retStr = ""; Log.i("vote", "doVote() voteStr:" + voteStr);
        try {
            StringBuffer stringBuffer = new StringBuffer();
//存储封装好的请求体信息
            stringBuffer.append("r=").append(URLEncoder.encode(voteStr, "utf-8"));
            byte[] data = stringBuffer.toString().getBytes();
            String urlPath = "http://10.64.26.242:8080/vote/GetVote";
...
//打开输入流，以便从服务器获取数据 //打开输出流，以便向服务器提交数据 //设置以Post方式提交数据 //使用Post方式不能使用缓存
//设置请求体的类型是文本类型
            ...
            if(response == HttpURLConnection.HTTP_OK) {
                ...
                retStr = inputStreamToString(inputStream); Log.i("vote", "retStr:" + retStr);
            }
        }
        catch (IOException e) {
            e.printStackTrace();
